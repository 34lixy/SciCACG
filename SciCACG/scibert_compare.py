...
class Model(nn.Module):
    def __init__(self, modelname, device, num_labels):
        ...
        self.avg_pooling_layer = nn.AvgPool2d(kernel_size=2, stride=2)
    ...
    def forward(self, p1, p2, p3):
        ...
        logits_p1 = logits1.last_hidden_state
        logits_p2 = logits2.last_hidden_state

        logits_max_p1 = self.max_pooling_layer(logits_p1)[:,0]
        logits_max_p2 = self.max_pooling_layer(logits_p2)[:,0]

        logits_avg_p1 = self.avg_pooling_layer(logits_p1)[:,0]
        logits_avg_p2 = self.avg_pooling_layer(logits_p2)[:,0]

        logits = torch.cat([logits_max_p1,logits_avg_p1,logits_max_p2,logits_avg_p2], dim=1)
...
for epoch in range(start_epoch, epochs + 1):
    ...
    if val_accuary < best_score:
        ...
        torch.save({"epoch": epoch,
                    ...
                    "recall": val_recall,
                    "f1": val_f1},
                   os.path.join('checkpoint', "bestca.pth.tar"))
...
# 加载最佳模型进行测试
checkpoint = torch.load("checkpoint/bestca.pth.tar")
...
save_path = os.path.join('fig', "compare_Matrix.png")
