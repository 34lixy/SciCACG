...
def train(model,
    ...
    for batch_index, batch in enumerate(tqdm_batch_iterator):
        ...
        native_paper_abstract = batch["native_paper_abstract"].to(device)
        native_paper_abstract_lengths = batch["native_paper_abstract_length"].to(device)
        ...
        output, logits, probs = model(citing_paper_abstract,
                                      citing_paper_abstract_lengths,
                                      cited_paper_abstract,
                                      cited_paper_abstract_lengths,
                                      native_paper_abstract,
                                      native_paper_abstract_lengths,
                                      cit_input)
        tgt_out = citation[:, 1:]
        loss1 = mask_loss_func(tgt_out, output)
        loss2 = criterion(logits,labels)
        loss = loss1 + loss2
        metric = mask_accuracy_func(tgt_out, output)
...
def validate(model, dataloader, criterion):
    ...
    with torch.no_grad():
        ...
        for batch_index, batch in enumerate(tqdm_batch_iterator):
            ...
            loss1 = mask_loss_func(cit_output, output)
            loss2 = criterion(logits,labels)
            loss = loss1 +loss2
            metric = mask_accuracy_func(cit_output, output)
            # 计算准确率
            correct_preds += correct_predictions(probs, labels)

            word_indices = output.cpu().numpy()
    ...
    epoch_loss = running_loss / len(dataloader)
    epoch_metric = running_metric / len(dataloader)
    accuracy = correct_preds / len(dataloader)
    return epoch_time, epoch_loss, epoch_metric, accuracy, rouge_sorce, rouge_sorce2
