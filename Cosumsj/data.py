...
class Preprocessor(object):
    ...
    def read_data(self, filepath):
        with (open(filepath, "r", encoding="utf8") as input_data):
            ...
            for line in input_data:
                ...
                native_paper_abstract = data["native_paper_abstract"]
                label = data["label"]
                ...
                if self.lowercase:
                    ...
                    native_paper_abstract = native_paper_abstract.lower()
                ...
                if self.ignore_punctuation:
                    ...
                    native_paper_abstract = native_paper_abstract.translate(str.maketrans("", "", string.punctuation))
                    native_paper_abstract = ''.join(
                        [c for c in native_paper_abstract if c not in string.punctuation and not c.isdigit()])
                ...
                native_words = [w for w in native_paper_abstract.rstrip().split() if w not in self.stopwords]
            ...
            native_paper_abstracts =[[word for word in words if word_frequency[word] > min_word_frequency] for words in native_paper_abstracts]
            return {"citing_paper_abstracts": citing_paper_abstracts,
                    "cited_paper_abstracts": cited_paper_abstracts,
                    "native_paper_abstracts":native_paper_abstracts,
                    "labels": labels,
                    "citation": citations}
    ...
    def build_worddict(self, data):
        ...
        [words.update(abstract) for abstract in data["native_paper_abstracts"]]
    ...
    def transform_to_indices(self, data):
        ...
        for i, abstract_1 in enumerate(data["citing_paper_abstracts"]):
            ...
            indices = self.words_to_indices(data["native_paper_abstracts"][i])
            transformed_data["native_paper_abstracts"].append(indices)
...
class ComparePaperDataset(Dataset):
    def __init__(self,
        ...
        self.native_paper_abstracts_lengths =[len(seq) for seq in data["native_paper_abstracts"]]
        self.citation_lengths = [len(seq) for seq in data["citation"]]

        if max_abstract_length == None:
            self.max_abstract_length = max(self.citing_paper_abstracts_lengths + self.cited_paper_abstracts_lengths+self.native_paper_abstracts_lengths)
        else:
        ...
        self.data = {
            ...
            "cited_paper_abstracts": torch.ones((self.num_sequences,
                                                 self.max_abstract_length),
                                                dtype=torch.long) * padding_idx,
            "native_paper_abstracts": torch.ones((self.num_sequences,
                                                 self.max_abstract_length),
                                                dtype=torch.long) * padding_idx,
        ...
        for i, native_paper_abstract in enumerate(data["native_paper_abstracts"]):
            end = min(len(native_paper_abstract), self.max_abstract_length)
            self.data["native_paper_abstracts"][i][:end] = torch.tensor(native_paper_abstract[:end])
    ...
    def __getitem__(self, index):
        ...
        return {
            ...
            "native_paper_abstract": self.data["native_paper_abstracts"][index],
            "native_paper_abstract_length": min(
                self.native_paper_abstracts_lengths[index], self.max_abstract_length
            ),
